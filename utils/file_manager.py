...
def windowing_image(arr, properties, window=None, use_normalize=True):
    if window is None:
        window_center = properties['window_center']
    ...
    image = arr * properties['rescale_slope'] + properties['rescale_intercept']
...
def windowing_volume(slices, arr, use_normalize=True):
    new_arr = []
    for dicom, image in zip(slices,arr):
        ...
        # # Normalize to 0-255
        # image = ((image - window_min) / dicom.WindowWidth * 255.0)
        # image = np.clip(image, 0, 255)
        
        # Normalize to 0-1 directly
        if use_normalize:
            image = (image - window_min) / dicom.WindowWidth
            image = np.clip(image, 0, 1)
...
def load_scan(path):
    ...
    total_length = len(slices) * properties['slice_thickness']
    if total_length >= 175:
        print("4 Tiles Stitching Data...")
        _type = 4
    elif total_length >= 130:
        print("2 Tiles Stitching Data...")
        _type = 2
    else:
        _type = 1
        print("1 Tile Data...")
